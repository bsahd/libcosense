...
class CosenseClient {
    ...
    async fetch(url, options) {
        ...
        return await (this.alternativeFetch ? this.alternativeFetch : fetch)(this.urlbase + url, usesessid
            ? {
                headers: {
...
class LatestPages {
    ...
    static async new(options, project) {
        return new LatestPages(await (await project.client.fetch("pages/" +
            project.name + "/?" +
            options.limit
            ? "limit=" + options.limit + "&"
            : "" +
...
class Page {
    ...
    static async new(pageName, project) {
        return new Page(await (await project.client.fetch(`pages/${project.name}/${encodeURIComponent(pageName)}`)).json(), project);
    }
    /**
...
class Project {
    ...
    static async useClient(projectName, client) {
        const projectInformation = await client.fetch("projects/" +
    ...
    async *pageList() {
        let followId = null;
        while (true) {
            const partialPageListResponse = await (await this.client.fetch("pages/" +
                this.name + "/search/titles" +
                (followId ? "?followingId=" + followId : ""))).json();
...
class SearchResult {
    ...
    static async new(query, project) {
        return new SearchResult(await (await project.client.fetch("pages/" + project.name +
            "/search/query?q=" + encodeURIComponent(query))).json(), project);
    }
    constructor(init, project) {
        Object.assign(this, init);
        this.project = project;
