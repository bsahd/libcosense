...
class LatestPages {
    ...
    static async new(options, project) {
        return new LatestPages(await (await project.client.fetch("pages/" + project.name + "/?" + options.limit
            ? "limit=" + options.limit + "&"
            : "" + options.skip
                ? "skip=" + options.skip + "&"
                : "" + options.sort
...
class SearchResult {
    ...
    static async new(query, project) {
        return new SearchResult(await (await project.client.fetch("pages/" +
            project.name +
            "/search/query?q=" +
            encodeURIComponent(query))).json(), project);
    }
    constructor(init, project) {
        Object.assign(this, init);
        this.project = project;
