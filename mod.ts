...
class CosenseClient implements CosenseClientopts {
   ...
   */
  fetch(url: string | URL, options?: RequestInit): Promise<Response> | Response {
...
class LatestPages {
   ...
   */
  static async new(options: LatestPagesInit = {}, project: Project): Promise<LatestPages> {
...
class Page {
  ...
  static async new(pageName: string, project: Project): Promise<Page> {
    return new Page(
      await (
        await project.client.fetch(`pages/${project.name}/${encodeURIComponent(pageName)}`)
      ).json(),
      project,
    );
  }
...
class Project {
   ...
   */
  static async useClient(projectName: string, client: CosenseClient): Promise<Project> {
    const projectInformation = await client.fetch("projects/" + encodeURIComponent(projectName));
  ...
  private constructor(name: string, client: CosenseClient, projectInfo: Project) {
    this.client = client;
    this.name = name;
    ...
    while (true) {
      const partialPageListResponse: PageListItem[] = await (
        await this.client.fetch(
          "pages/" + this.name + "/search/titles" + (followId ? "?followingId=" + followId : ""),
        )
      ).json();
...
class SearchResult {
  ...
  static async new(query: string, project: Project): Promise<SearchResult> {
    return new SearchResult(
      await (
        await project.client.fetch(
          "pages/" + project.name + "/search/query?q=" + encodeURIComponent(query),
        )
      ).json(),
      project,
    );
